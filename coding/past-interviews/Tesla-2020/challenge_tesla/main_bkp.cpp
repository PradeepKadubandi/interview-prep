...
class Node {
    public:
        shared_ptr<forward_list<pair<shared_ptr<Node>, double> > > _edges;

        Node(row data) : _data(data) {
            _edges = make_shared<forward_list<pair<shared_ptr<Node>, double> > >();
        }

        bool operator ==(const Node& other) const{
            return _data.name == other._data.name;
        }

        void add_edge(shared_ptr<Node> other, double cost) {
            _edges->push_front(make_pair(other, cost));
        }
    ...
    public:
        ...
        void print() {
            ...
            for (auto p : _name_to_node_map) {
                cout << p.first << " --> [ ";
                for (auto e : *(p.second->_edges)) {
                    cout << e.first->get_data().name << ", " << e.second << ";";
                }
                cout << endl;
            }
            cout << "-------------------------------------------------" << endl;
    ...
    for (auto it1 = network.begin(); it1 < network.end(); ++it1) {
        for (auto it2 = it1+1; it2 < network.end(); ++it2) {
            ...
            if (distance <= FULL_RANGE) {
                ...
                n1->add_edge(n2, distance);
                n2->add_edge(n1, distance);
                edgeCount += 1;
            }
        }
    }
...
shared_ptr<Result> findMinimumTimePath(shared_ptr<Node> start, shared_ptr<Node> destination, shared_ptr<unordered_map<string, double> > visited, shared_ptr<list<IntermediateStation > > path) {
    auto curr_charger = start->get_data();
    if (curr_charger.name == destination->get_data().name) {
    ...
    for (auto edge : *start->_edges) {
        auto remRange = visited->at(curr_charger.name);
        if (visited->find(edge.first->get_data().name) == visited->end()) {
            auto next_charger = edge.first->get_data();
            ...
            additional_time = edge.second / SPEED;
            if (edge.second > remRange) {
                charge_time = (edge.second - remRange) / curr_charger.rate;
                additional_time += charge_time;
                next_rem_range = 0.0;
            }
            else {
                charge_time = 0.0;
                next_rem_range = remRange - edge.second;
            }
            visited->emplace(next_charger.name, next_rem_range);
            path->push_back(IntermediateStation(next_charger.name, charge_time));
            min_path_from_next = findMinimumTimePath(edge.first, destination, visited, path);
            visited->erase(next_charger.name);
            path->pop_back();
