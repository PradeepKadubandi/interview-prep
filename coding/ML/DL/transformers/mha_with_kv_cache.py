...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
            ...
            past_keys: Optional[torch.Tensor] = None, # B, n, S_past, D
            past_values: Optional[torch.Tensor] = None, # B, n, S_past, D
        ...
        if past_keys is not None:
            ...
            total_k = torch.cat((past_keys, k_values), dim = 2) # B, n, S_past + S_k, D
            total_v = torch.cat((past_values, v_values), dim = 2) # B, n, S_past + S_k, D
        else:
            total_k, total_v = k_values, v_values

        scores = torch.einsum("bnqd,bnsd->bnqs", q_values, total_k)
        # return scores
        scores = F.softmax(scores / math.sqrt(self.head_dim), dim=-1)

        scores = torch.einsum("bnqs,bnsd->bnqd", scores, total_v)
        scores = self.reshape_from_heads(scores)
        # return scores
        
        o_values = self.o_proj(scores)
        return o_values + (total_k, total_v) # use_cache is being ignored here...
